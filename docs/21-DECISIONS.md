...
| D-055 | A movimentação pendente de aprovação é uma linha de `transactions` com `status = 'PENDING_APPROVAL'`, não uma cópia em `approval_requests` | A tabela já previa o status desde a 0010 e TODO cálculo — saldo, limite do cartão, relatórios — soma apenas `POSTED`. Assim "não afeta saldo enquanto pendente" (docs/04 §16) não precisa de nenhuma regra nova, e a pendência aparece na 1g como o SCREEN-SPECS pede. `approval_requests` guarda só o que a movimentação não sabe: quem pediu, qual regra estava valendo, quem decidiu. |
| D-056 | A regra de aprovação vale para **despesa**, não para receita | docs/04 §16 fala em "movimentações sujeitas a aprovação" sem distinguir, mas supervisão existe para controlar o que SAI. Exigir autorização para o filho registrar a própria mesada seria atrito sem propósito. Compra em cartão entra na regra porque também é gasto. |
| D-057 | O limite de aprovação é **inclusivo** e, sem valor gravado, `ABOVE_THRESHOLD` se comporta como `ALWAYS` | A copy da 8b é "Valor limite sem aprovação · R$ 50,00", então gastar exatamente 50 passa. Falhar ABERTO na ausência do limite transformaria um dado faltante em permissão irrestrita — regra de supervisão precisa falhar fechada. |
| D-058 | Ninguém decide o próprio pedido, mesmo tendo perfil para decidir | A matriz de STATES-AND-MATRICES §2 diz quem aprova, não que se possa autoaprovar. Um adulto marcado como supervisionado que aprovasse a si mesmo anularia a própria supervisão. |
| D-059 | O `maxHeight: '88%'` do BottomSheet mora no container, não na folha | Uma porcentagem se resolve contra a altura do pai; o pai da folha era dimensionado pela própria folha, e o Yoga resolvia a circularidade encolhendo. Qualquer folha com conteúdo acima de ~59% da tela perdia o fim do conteúdo — na 3c, o campo de mensagem. Encontrado no gate da 3c e visível também na 2d, já "aprovada" antes com 63dp de sobra no rodapé. |
| D-060 | A lista de aprovações (que o link da 3a abre) é montada com `SectionLabel` + `Card` de `ListRow`, como a 3a e a 3d | O pacote só desenha a folha de decisão (3c). Regra 8 do CLAUDE.md: sem screenshot e sem COMPONENT-SPECS, seguir o mais próximo do existente. O desfecho aparece como "● Aprovada por Ana", com o ● junto do texto — mesmo padrão da linha de convite pendente da 3a. |
| D-061 | `spec-values.sheetMoney` (fonte 30) para o valor proposto da 3c | SCREEN-SPECS §3c pede "valor expense 30"; `moneyLg` é 32 e `moneyMd`, 20. A medição de `3c-aprovacao.png` confirma: 21dp de altura de caixa alta, que em Manrope ExtraBold corresponde a 30. Mesmo tratamento dado ao `loginLogo`. |
| D-063 | WatermelonDB com o adaptador **pelo bridge**, sem `jsi: true` | O instalador JSI da versão 0.28 pede o ponteiro do runtime pelo caminho antigo, que o modo bridgeless do React Native 0.86 não expõe. O adaptador padrão passa pelo módulo nativo clássico, que a camada de interoperabilidade do bridgeless atende, e o volume aqui — algumas centenas de linhas de cache — está muito abaixo do ponto em que o ganho do JSI apareceria. |
| D-064 | O cache guarda o **JSON do contrato inteiro** em `payload`, com poucas colunas espelhadas só para consultar e ordenar | Reidratar devolve exatamente o objeto que o servidor mandou; um campo novo no contrato já vem junto, sem migração de schema local. E deixa explícito que o cache não CALCULA nada: saldo, fatura e "vencido" continuam derivados no Postgres. |
| D-065 | A chave de idempotência do item de outbox nasce no **enfileiramento**, não no envio | É o que torna o reenvio seguro justamente no caso em que importa: quando não se sabe se o primeiro envio chegou. Gerada no envio, uma retentativa criaria uma segunda movimentação. |
| D-066 | `flush` **para** no primeiro erro de rede, em vez de seguir para os próximos itens | A ordem da fila importa (uma conta prevista criada offline pode ser quitada em seguida) e insistir com a rede caída só gasta bateria. Erro definitivo do servidor, esse sim, pula para o próximo. |
| D-067 | Sincronização disparada por evento, sem laço de retentativa em segundo plano | Três gatilhos: volta ao primeiro plano, primeira requisição bem-sucedida depois de uma queda, e o "Tentar" da faixa. A primeira requisição que funciona é o sinal mais barato de que a rede voltou — mais confiável que um temporizador e sem custar uma dependência nativa de conectividade. |
